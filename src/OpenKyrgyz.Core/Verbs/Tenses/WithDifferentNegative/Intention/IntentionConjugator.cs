...
namespace OpenKyrgyz.Core.Verbs.Tenses.WithDifferentNegative.Intention;

public class IntentionConjugator
{
        ...
        VerbFormEnum form)
    {
        ...
        {
            verb += IntentionEnding.GetIntentionEnding(verb);
        }
    ...
    {
        var ending = IntentionEnding.GetEnding(verb, pronoun);
    ...
    private static string ConjugateForАлар(string verb, VerbFormEnum form)
    {
        ...
        var ending = IntentionEnding.GetEnding(verb, PronounEnum.Алар);
        verb += ending;
